...
def generalFeats(file_TS,window_size,overlap,mode):
    ...
    for n_index in file_TS.index:
        ...
        amplLoc = np.max(value_loc)-np.min(value_loc)
        maxLoc = np.max(value_loc)
        ...
        if overlap:
            medianAfter, meanAfter, sdAfter, medianAfterwind,\
        ...
        file_info.append([label,
                          ...
                          medianAfterwind, meanAfterwind,
                          mean_medianBef, mean_medianWind,
                          sdLoc, amplLoc, skewLoc,
                          maxLoc, medianLoc, meanLoc])
        
        df_add = pd.DataFrame(file_info,
                          columns=['label',
                                   'sdBefore_'+mode, 'sdWind_'+mode,
                                   'medianAfter_'+mode, 'meanAfter_'+mode,
                                   'medianAfterwind_'+mode, 'meanAfterwind_'+mode,
                                   'mean_medianBef_'+mode, 'mean_medianWind_'+mode,
                                   'sdLoc_'+mode, 'amplLoc_'+mode, 'skewLoc_'+mode,
                                   'maxLoc_'+mode, 'medianLoc_'+mode, 'meanLoc_'+mode])
    ...
    medianTot = pd.DataFrame([medianAfter]*len(file_info),
                             columns=['medianTot_'+mode])
    ...
    mean_medianTot = pd.DataFrame([mean_medianBef]*len(file_info),
                                  columns=['mean-medianTot_'+mode])
    ...
    df_add = pd.concat(
        [df_add, medianTot, meanTot, sd_meanTot, sd_maxTot, skew_meanTot,
         mean_medianTot, mean_maxTot], axis=1
        )
...
def update_after(list_values, window_size, step, overlap):
    ...
    """
    medianAfter = np.ma.median(list_values)
    ...
    medianAfterwind = np.ma.median(list_values_afterwind)
    meanAfterwind = np.mean(list_values_afterwind)

    return medianAfter, meanAfter, sdAfter, medianAfterwind, meanAfterwind
